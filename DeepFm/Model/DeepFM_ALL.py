...
def model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope("Deep-part"):
        ...
        deep_inputs_p = tf.reshape(embeddings,shape=[-1,field_size*embedding_size]) # None * (F*K)
        deep_inputs=deep_inputs_p
...
def main(_):
    idx=0
    mid_feat=[]
    tr_files = glob.glob("%s/tr1.libsvm" % FLAGS.data_dir)
    random.shuffle(tr_files)
    print("tr_files:", tr_files)
    va_files = glob.glob("%s/va1*libsvm" % FLAGS.data_dir)
    print("va_files:", va_files)
    te_files = glob.glob("%s/te1*libsvm" % FLAGS.data_dir)
    ...
    if FLAGS.task_type == 'train':
        ...
        for i, prob in enumerate (preds):
            idx += 1
            mid_feat.append(list(prob['prob']))
            print("prod:",prob['prob'])
            print ("prod:", prob['prob'].shape)
            if idx>=999:
                with open (FLAGS.data_dir + "/middle_deepout.txt", "w") as fo:
                    for i in mid_feat:
                        feat=list(map(lambda x:str(x),i))
                        fo.write (' '.join(feat))
                        fo.write ('\n')
                    return
        # ids = []
        ...
        # with open (FLAGS.data_dir + "/pred_dey.csv", "w") as fo:
        #     for i, prob in enumerate (preds):
        #         fo.write ("%s,%0.3f\n" % (str (ids[i]), prob['prob']))

...
tf.app.flags.DEFINE_integer("embedding_size", 32, "Embedding size")# 0520 and 0521 (32)
...
tf.app.flags.DEFINE_float("learning_rate", 0.001, "learning rate")  # 0520(0.0001)  0521(0.0001)
tf.app.flags.DEFINE_float("l2_reg", 0.01, "L2 regularization") # 0520(0.001)  0521(0.01)
tf.app.flags.DEFINE_string("loss_type", 'log_loss', "loss type {square_loss, log_loss}")
tf.app.flags.DEFINE_string("optimizer", 'Adam', "optimizer type {Adam, Adagrad, GD, Momentum}")
tf.app.flags.DEFINE_string("deep_layers", '256,128,64', "deep layers")  # 0520 and 0521 ('256,128,64')
tf.app.flags.DEFINE_string("dropout", '0.70,0.70,0.50', "dropout rate") ## 0520(0.9 0.9 0.9)  0521(0.7 0.7 0.5)
tf.app.flags.DEFINE_boolean("batch_norm", True, "perform batch normaization (True or False)")
tf.app.flags.DEFINE_float("batch_norm_decay", 0.9, "decay for the moving average(recommend trying decay=0.9)")

tf.app.flags.DEFINE_string("data_dir", '/home/ganhy/mnt_usb/deepfm/Data', "data dir")
tf.app.flags.DEFINE_string("dt_dir", '', "data dt partition")
tf.app.flags.DEFINE_string("model_dir", '/home/ganhy/mnt_usb/deepfm/Model/model', "model check point dir")
tf.app.flags.DEFINE_string("servable_model_dir", '', "export servable model for TensorFlow Serving")
tf.app.flags.DEFINE_string("task_type", 'infer', "task type {train, infer, eval, export}")
...
def model_fn(features, labels, mode, params):
    ...
    FM_V = tf.get_variable(name='fm_v', shape=[feature_size,field_size,embedding_size], initializer=tf.glorot_normal_initializer())
...
def main(_):
    ...
    config = tf.estimator.RunConfig().replace(session_config = tf.ConfigProto(device_count={'GPU':5}),
    ...
    if FLAGS.task_type == 'train':
        train_spec = tf.estimator.TrainSpec(input_fn=lambda: input_fn(tr_files, num_epochs=FLAGS.num_epochs, batch_size=FLAGS.batch_size),max_steps=81000)#135000

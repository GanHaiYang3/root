...
continous_features = range(2, 15)
categorial_features = range(15, 41)
    ...
    def build(self, datafile, categorial_features, cutoff=0):
        with open(datafile, 'r') as f:
            for line in f:
                ...
                if (len (features) < self.num_feature):
                    continue
...
def preprocess(datadir, outdir):
    ...
    for i in range(1, len(categorial_features)+1):
        ...
        total=total+dict_sizes[i - 1]
        print("i:",i-1,"dict_sizes",dict_sizes[i - 1])
    ...
    with open(FLAGS.output_dir + 'tr1_new.libsvm', 'w') as out_train:
        with open(FLAGS.output_dir + 'va1_new.libsvm', 'w') as out_valid:
    ...
    # with open(FLAGS.output_dir + 'pre_new.libsvm', 'w') as out:
    #     with open(FLAGS.input_dir + 'pre.txt', 'r') as f:
    #         for line in f:
    #             features = line.rstrip('\n').split(',')
    #             print(len (features))
    #             if (len (features) < 41):
